...
def mars_surface():
    ...
    land[landing_site, 0] = randint(200, 500)
    land[landing_site+1, 0] = min(land[landing_site, 0] + randint(100, 200), 699)
    land[landing_site+1, 1] = land[landing_site, 1] = randint(1, 150)
    
    # fill in the rest of the terrain
    for i in range(landing_site):
        land[i, 0] = (land[landing_site, 0] / landing_site) * i
        land[i, 1] = randint(0, 150)
    
    for i in range(landing_site + 2, surfaceN):
        land[i, 0] = (land[landing_site + 1, 0] + 
                      (700 - land[landing_site + 1, 0]) / len(land[landing_site + 2:]) * 
                      (i - (landing_site + 1)))
        land[i, 1] = randint(0, 150)
