...
def simulate(r, t, y, u, X0, V0, land, landing_site, 
 ...
 # change in velocity would be (Vs[i, :] - Vs[i-1, :]) / dt
        ...
        if autopilot is not None:
            ...
            rotate, power, e = autopilot(r, t, y, u, Es, i, X, V, fuel, rotate, power)
            Es[i] = e
    ...
    return Xs[:Nstep,:], Vs[:Nstep,:], thrust[:Nstep,:], success, Es[:Nstep], fuel
...
def proportional_autopilot(Kh, Kp, Kd, Ki, Es, i, X, V, fuel, rotate, power):
    ...
    dIntegral = 0.5*dt*(Es[0] + Es[i] + 2*(sumERROR)) # integral component using Trapezium Rule
    ...
    #diagram of geometry
    Pout = K_p*e + K_d*dE + K_i*dIntegral
    power = min(max(Pout, 0.0), 5)
    
    # 2 D.O.F control (tuning)
    # either generalised error, then PID
    # or PID for vertical case
    '''
...
'''
X0 = [(land[landing_site+1, 0] + land[landing_site, 0]) // 2, 5000]
V0 = [0., 0.]
Xs, Vs, thrust, success, Es = simulate(X0, V0, land, landing_site, dt=0.1, Nstep=3000, 
...
khlist = np.arange(0.001, 0.05, 0.005, dtype=float).tolist()
kplist = np.arange(0.01, 1, 0.01, dtype=float).tolist()
kdlist = np.arange(0.001, 0.05, 0.005, dtype=float).tolist()
kilist = np.arange(0.001, 0.05, 0.005, dtype=float).tolist()

# can keep the proportional constants the same as previous case for now
...
for r in khlist:
    for t in kplist:
        for y in kdlist:
            for u in kilist:
                Xs, Vs, thrust, success, Es, fuel = simulate(r, t, y, u, X0, V0, land, landing_site, dt=0.1, Nstep=2000,
                                         autopilot=proportional_autopilot, fuel=1000)

                if success == True:
                    output.append((r,t,y,u,fuel,Vs[-1,1]))
...
for p in output:
    finalVelocities.append(p[5])

print(finalVelocities.index(max(finalVelocities)))
