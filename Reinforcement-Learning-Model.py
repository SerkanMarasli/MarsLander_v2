...
SHOW_EVERY = 250   # how often to play through env visually.
...
for episode in range(HM_EPISODES):
    ...
    if moving_avg[-1] >= 9000:
        ninekcount += 1
    if ninekcount >= 10:
        if episode_reward >= 8950:
            if ninekcount % 1250 == 0:
                plot_lander(land, landing_site, Xhist[:i])
                plt.title(f'episode reward = {episode_reward}')
                plt.savefig(f'Figures2/count_{counter:05d}_ep_{episode:05d}_reward_{episode_reward:05d}.png')
                counter += 1
...
fig, ax = plt.subplots()
ax.plot([i for i in range(len(moving_avg))], moving_avg, 'b')
...
np.save("q_tableNEW-50by50", q_table)
